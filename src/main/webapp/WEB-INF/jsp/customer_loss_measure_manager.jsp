...
$(function() {
	...
	$.post("${ctx}/customerLossMeasure/findCustomerLossById.action",
		...
		function (data) {
			if (data.status==0) {
				$("#customerNoId").﻿﻿textbox('setValue',data.data.customerName),
				$("#customerNameId").﻿﻿textbox('setValue',data.data.customerName),
				$("#customerManagerId").﻿﻿textbox('setValue',data.data.customerManager),
				$("#lastOrderTimeId").﻿﻿textbox('setValue',data.data.lastOrderTime)
			}
		},"json"		
	);
	/*展示数据的datagrid表格*/
...
function openTab(id) {
	...
	function openUpdateDialog() {
	 ...
	 /* 开发成功或失败 */
	 function updateSaleChance(status) {
			$.post("${ctx}/customerLossMeasure/customerLossStatus.action",
				{"id":'${param.lossId}',"status":status,"confirmLossTime":getCurrentDateTime()},
				function(data) {
		...
		<div id="toolbar">
			<c:if test="${param.show != 'true' }">
					<div> 
						...
						<a class="easyui-linkbutton" href="javascript:updateSaleChance(1)" iconCls="icon-zzkf">确定流失</a>
					</div>
			</c:if>
		</div>
	<!-- toolbar 结束 -->
