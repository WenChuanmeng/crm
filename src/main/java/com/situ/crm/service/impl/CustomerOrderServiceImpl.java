...
public class CustomerOrderServiceImpl implements ICustomerOrderService{
	...
	public DataGridResult findAll(Integer page, Integer rows, Integer customerId) {
		...
		//2、执行查询
		//rows(分页之后的数据)
		com.situ.crm.pojo.CustomerOrderExample.Criteria createCriteria = customerOrderExample.createCriteria();
		if (customerId != null) {
			createCriteria.andCustomerIdEqualTo(customerId);
		}
		
		//查询状态为已分配的 
		List<CustomerOrder> selectByExample = customerOrderMapper.selectByExample(customerOrderExample);
		//total
		PageInfo<CustomerOrder> pageInfo = new PageInfo<>(selectByExample);
	...
	public ServerResponse deleteById(Integer id) {
		...
		int result = customerOrderMapper.deleteByPrimaryKey(id);
		if (result > 0) {
